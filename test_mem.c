...
void run_all_tests() {
    ...
    // Test 1: Test exceeding single allocation
    printf("Running test: Exceed single allocation...\n");
    test_exceed_single_allocation_multithread((TestParams){.memory_size = 1024});

    // Test 2: Test exceeding cumulative allocation
    printf("Running test: Exceed cumulative allocation...\n");
    for (int i = 1; i < 6; i++) {
        test_exceed_cumulative_allocation_multithread((TestParams){.num_threads = pow(2, i), .memory_size = pow(2, 11 + i)});
    }

    // Test 3: Memory overcommit test
    printf("Running test: Memory overcommit...\n");
    test_memory_overcommit_multithread((TestParams){.memory_size = 1024});

    // Test 4: Repeated fit and reuse
    printf("Running test: Repeated fit reuse...\n");
    test_repeated_fit_reuse_multithread((TestParams){.iterations = 1});

    // Test 5: Memory fragmentation test
    printf("Running test: Memory fragmentation...\n");
    test_memory_fragmentation_multithread((TestParams){.iterations = 1});

    // Test 6: Random block allocation test
    printf("Running test: Random block allocation...\n");
    test_random_blocks_multithread((TestParams){.memory_size = 1024, .block_size = 1024});
